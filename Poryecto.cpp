...
class List{
	...
	public:
		...
		void addPost(Node<T> *pos,T payload){	
			...
			if(first == NULL){//Si la lista estï¿½ vacï¿½a, lo asignamos y ya
				first = last = aux;
				return;
			}
			
			Node<T> *aux2 = first;//En caso de que la lista no estï¿½ vacï¿½a, creamos un puntero para buscar la posiciï¿½n P en donde quiere que lo insertemos
			...
			if(aux2 == NULL){//Si el elemento no estï¿½ en la lista, lo aï¿½adimos de ultimo
				aux->previous = last;
				last->next = aux;
				last = aux;
				aux->next = NULL;
		...
		void addPre(Node<T> *pos,T payload){
			...
			if(aux2 == NULL){//Si el elemento no estï¿½ en la lista, lo aï¿½adimos de ï¿½ltimo
				aux->previous = last;
				last->next = aux;
				last = aux;
				aux->next = NULL;
...
class PriorityList{
	...
	public:
		...
		void add(Node<T> *payload){
			if(lista.isEmpty()){
				lista.addPre(NULL,payload->payload);//Si la lista estï¿½ vacï¿½a, simplemente lo inserto y ya
				return;
			}
